...
describe("models/shelf", () => {
  ...
  describe("fromEncodingQuery", () => {
    it("throws error for autocount query", () => {
      ...
      expect(() => fromEncodingQuery(autoCountQuery)).toThrowError(
        "AutoCount Query not yet supported"
      );
    });
  });
  ...
  describe("fromValueQuery", () => {
    it("throws error for wildcard value", () => {
      ...
      expect(() => fromValueQuery(fieldQuery)).toThrowError(
        "Voyager does not support wildcard value"
      );
    });
  ...
  describe("fromFieldQueryNestedProp", () => {
    it("throws error for boolean", () => {
      ...
      expect(() => fromFieldQueryNestedProp(fieldQuery, "scale")).toThrowError(
        "Voyager does not support boolean scale"
      );
    });
    ...
    it("throws error for wildcard", () => {
      ...
      expect(() => fromFieldQueryNestedProp(fieldQuery, "scale")).toThrowError(
        "Voyager does not support wildcard scale"
      );
    });
    ...
    it("throws error for scale with wildcard", () => {
      ...
      expect(() => fromFieldQueryNestedProp(fieldQuery, "scale")).toThrowError(
        "Voyager does not support wildcard scale type"
      );
    });
  });
});
