...
export function isWildcardChannelId(
  shelfId: ShelfId
): shelfId is ShelfWildcardChannelId {
...
export function fromFieldQuery(fieldQ: FieldQuery): ShelfFieldDef {
  ...
  if (isWildcard(type)) {
    ...
    console.warn(
      "Voyager does not support ordinal type yet, converting to nominal"
    );
    type = "nominal";
  }
