...
import { isLocalSingleTimeUnit as isTimeUnit } from "vega-lite/build/src/timeunit";
...
export type FieldQueryFunctionMixins = Pick<
  FieldQuery,
  "aggregate" | "timeUnit" | "bin" | "hasFn"
>;
...
export function toFieldQueryFunctionMixins(
  ...
  if (isWildcard(fn)) {
    ...
    const functionTypeCount =
      (aggregates.length > 0 ? 1 : 0) +
...
export function fromFieldQueryFunctionMixins(
  ...
  if (aggregate) {
    if (isWildcard(aggregate)) {
      const aggregates = isShortWildcard(aggregate)
        ? config.enum.aggregate
        : aggregate.enum;
      fns = fns.concat(
      ...
      hasUndefinedInEnum =
        hasUndefinedInEnum || contains(aggregates, undefined);
    } else if (!fn) {
      fn = aggregate;
    } else {
  ...
  if (timeUnit) {
    if (isWildcard(timeUnit)) {
      const timeUnits = isShortWildcard(timeUnit)
        ? config.enum.timeUnit
        : timeUnit.enum;
